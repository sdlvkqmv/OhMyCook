...
import { UserSettings, Recipe, RecipeFilters } from '../types';
      ...
      substitutions: {
        ...
        items: {
          ...
          properties: {
            missing: {
              type: Type.STRING,
              description: 'The ingredient the user is missing.'
            },
            substitute: {
              type: Type.STRING,
              description: 'A suggested substitute for the missing ingredient.'
            }
          },
...
export async function getRecipeRecommendations(ingredients: string[], priorityIngredients: string[], filters: RecipeFilters, language: 'en' | 'ko'): Promise<Recipe[]> {
  ...
  try {
    ...
    throw new Error("Failed to get recipes from AI. Please check your ingredients and try again.");
  }
}
...
export async function analyzeReceipt(base64Image: string): Promise<string[]> {
    ...
    try {
        ...
        throw new Error("Failed to analyze receipt image. Please try again with a clearer picture.");
    }
}

export async function chatWithAIChef(history: {role: string, parts: {text: string}[]}[], message: string, settings: UserSettings, language: 'en' | 'ko', recipeContext?: Recipe | null): Promise<string> {
    const model = 'gemini-2.5-pro';
    ...
    try {
        ...
        console.error("Error chatting with AI Chef:", error);
        throw new Error("The AI Chef is busy chopping vegetables. Please try again in a moment.");
    }
}